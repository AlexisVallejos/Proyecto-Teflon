...
const ShortcutHint = ({ keys, label }) => (
    ...
    >
        <span className="text-xs font-medium admin-text-primary">{label}</span>
...
const CommandItem = ({ icon: Icon, label, onSelect, shortcut }) => (
    <Command.Item
        onSelect={onSelect}
        className="group flex cursor-pointer items-center justify-between rounded-xl px-3 py-2.5 text-xs font-semibold admin-text-primary transition-all aria-selected:bg-[var(--admin-accent-soft-strong)] aria-selected:text-[var(--admin-text)]"
    >
        ...
        {shortcut ? (
            <div className="flex items-center gap-1 opacity-70 transition-opacity group-aria-selected:opacity-100">
...
const CommandPalette = ({ branding, onAddItem }) => {
    ...
    return (
        ...
        >
            ...
            <Command
                className="admin-panel-surface relative w-full max-w-[600px] overflow-hidden rounded-2xl border shadow-2xl ring-1 ring-black/5 animate-in zoom-in-95 duration-300"
            >
                <div
                ...
                >
                    ...
                    <Command.Input
                        ...
                        className="flex-1 border-none bg-transparent text-[13px] font-medium outline-none placeholder:text-zinc-500 admin-text-primary"
                    />
                    <div
                ...
                <Command.List className="custom-scrollbar max-h-[360px] overflow-auto p-2">
                    ...
                    <Command.Group heading="Atajos" className="px-2 py-2">
                        <div className="grid gap-2 px-1">
                            <ShortcutHint keys={['Ctrl', 'K']} label="Abrir comandos" />
                            <ShortcutHint keys={['Esc']} label="Cerrar paneles" />
                            <ShortcutHint keys={['Enter']} label="Seleccionar resultado" />
                            <ShortcutHint keys={['↑', '↓']} label="Mover seleccion" />
                        </div>
                    </Command.Group>
                </Command.List>
